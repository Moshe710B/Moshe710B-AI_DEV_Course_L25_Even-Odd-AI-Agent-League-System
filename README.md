...
|------|-------------|-------|
| `SIMULATION_run_league.py` | League simulation entry point | 52 |
...
| File | Description | Lines |
|------|-------------|-------|
| `agents/league_manager/main.py` | League Manager agent | 155 |
| `agents/league_manager/handlers.py` | LM message handlers | 322 |
| `agents/league_manager/scheduler.py` | Round-robin scheduler | 133 |
| `agents/league_manager/standings.py` | Standings calculator | 121 |
| `agents/referee_template/main.py` | Referee agent template | 136 |
| `agents/referee_template/handlers.py` | Referee message handlers | 185 |
| `agents/referee_template/game_logic.py` | Match orchestration | 159 |
| `agents/referee_template/invitation_handler.py` | Player invitation logic | 114 |
| `agents/referee_template/parity_handler.py` | Parity collection logic | 141 |
| `agents/player_template/main.py` | Player agent template | 159 |
| `agents/player_template/handlers.py` | Player message handlers | 200 |
| `agents/player_template/state.py` | Player state management | 116 |
| `agents/player_template/strategies/base.py` | Strategy interface | 48 |
| `agents/player_template/strategies/random_strategy.py` | Random strategy | 25 |
| `agents/player_template/strategies/deterministic.py` | Deterministic strategy | 41 |
| `agents/player_template/strategies/alternating.py` | Alternating strategy | 40 |
| `agents/player_template/strategies/adaptive.py` | Adaptive learning strategy | 69 |
| `agents/player_template/strategies/llm_strategy.py` | LLM-based strategy | 123 |
...
## Running the Full League (Distributed Mode)
...
- Wait for 4 players and 1+ referees to register
- Auto-start the league **60 seconds** after minimum requirements are met
- Display match results and standings in real-time
...
**Important**: All agents must register within 60 seconds of the minimum being met, or you'll need to restart them.

### Alternative: PowerShell Script (Windows)
```powershell
.\start_league.ps1
```
This automatically starts all 7 agents in separate windows.
...
```bash
# Sequential execution (matches run one at a time)
python SIMULATION_run_league.py

# Parallel execution (faster, matches run concurrently)
python SIMULATION_run_league.py --parallel
```

The simulation completes in under 1 second and shows:
- Match results for all 6 games (3 rounds)
- Standings after each round
- Final champion and statistics
...
**Full Product Design Review**: [DOCS/PRD_EvenOdd_League.md](DOCS/PRD_EvenOdd_League.md) - Complete specification and design
...
Moshe Bellaiche and Hadar Wayn
...
### League Manager shows "MATCH_STARTED" but no results

**Symptom**: Matches start but never complete, no standings updates.

**Solution**: Check if referee is successfully reporting results:
```bash
# Check referee logs
cat SHARED/logs/referees/REF01.log.jsonl | grep "RESULT_REPORTED"

# Check for auth failures in league manager
cat SHARED/logs/league_manager/MANAGER.log.jsonl | grep "AUTH_FAILED"
```

If you see `AUTH_FAILED: referee:REF01`, the auth tokens weren't registered properly. This was fixed in the latest version.

### Empty standings table

**Symptom**: Round completes but standings table shows no data.

**Solution**: This was a key mismatch bug (fixed). Make sure you're using the latest version where standings use `wins/draws/losses` (plural) instead of `won/draw/lost`.

### Port already in use

**Symptom**: `Address already in use` error when starting agent.
...
### Simulation works but distributed mode fails

**Symptom**: `SIMULATION_run_league.py` works perfectly, but distributed agents don't complete matches.

**Common causes**:
1. **Auth tokens not registered** - Fixed in latest version
2. **Async handlers blocking** - Referees must use background tasks for match orchestration
3. **Network timeouts** - Check firewall settings, ensure localhost is accessible
4. **Wrong endpoints** - Verify player/referee endpoints in registration logs

### All matches result in TECHNICAL_LOSS

**Symptom**: Every match shows `TECHNICAL_LOSS` in results.

**Diagnosis**:
```bash
# Check referee logs for the exact failure point
cat SHARED/logs/referees/REF01.log.jsonl | grep -E "(INVITATION_FAILED|JOIN_CHECK|TECHNICAL_LOSS)"
```

**Common causes**:
- Players not responding to `GAME_INVITATION`
- Incorrect player endpoints
- Auth validation blocking player messages

### Viewing Logs

All agents log to `SHARED/logs/` in JSONL format:
```bash
# League Manager
cat SHARED/logs/league_manager/MANAGER.log.jsonl | tail -50

# Referees
cat SHARED/logs/referees/REF01.log.jsonl | tail -50
cat SHARED/logs/referees/REF02.log.jsonl | tail -50

# Players
cat SHARED/logs/players/P01.log.jsonl | tail -50
cat SHARED/logs/players/P02.log.jsonl | tail -50
```
