...
```bash
python SIMULATION_run_league.py              # Sequential execution
python SIMULATION_run_league.py --parallel   # Parallel execution (faster)
```
...
cat SHARED/logs/league_manager/MANAGER.log.jsonl | tail -50

# View referee logs (most useful for debugging match flow)
cat SHARED/logs/referees/REF01.log.jsonl | tail -100
```
...
The system supports running multiple matches concurrently using multiple referee instances. The `SIMULATION_run_league.py` script demonstrates this with `--parallel` flag.
...
| Aspect | Simulation Mode (`SIMULATION_run_league.py`) | Distributed Mode (HTTP agents) |
|--------|-----------------------------------|--------------------------------|
...
This section documents key issues encountered during distributed mode implementation and their solutions.

### Bug Fix #1: Parity Response Not Captured

**Problem**: Referee sent `CHOOSE_PARITY_CALL` to players but ignored the HTTP response, waiting 30 seconds for nothing.

**Root Cause**: Code was calling `await client.send()` but not capturing the return value.

**Solution**: Capture and parse the HTTP response immediately:
```python
# BEFORE (broken)
await client.send(endpoint, "CHOOSE_PARITY_CALL", {...})
await asyncio.sleep(30)  # Wait for nothing!
...
**File**: `agents/referee_template/parity_handler.py`

### Bug Fix #2: Auth Tokens Not Registered

**Problem**: Referees sent `MATCH_RESULT_REPORT` to League Manager but got `AUTH_FAILED` rejection.

**Root Cause**: League Manager generated auth tokens during registration but never called `register_auth_token()` to add them to the validator.

**Solution**: Register tokens immediately after generation:
```python
# In League Manager registration handlers
auth_token = generate_token()
self.manager.registered_players[player_id] = {..., "auth_token": auth_token}
self.manager.server.register_auth_token(f"player:{player_id}", auth_token)  # ADD THIS!
```

**Files**: `agents/league_manager/handlers.py`

### Bug Fix #3: Player→Referee Messages Blocked by Auth

**Problem**: Players sent `GAME_JOIN_ACK` and `CHOOSE_PARITY_RESPONSE` to referees but got rejected.

**Root Cause**: These messages required auth validation, but referees don't have player auth tokens (only League Manager has them).

**Solution**: Remove these message types from `AUTH_REQUIRED_MESSAGE_TYPES`:
```python
# Only require auth for messages TO League Manager
AUTH_REQUIRED_MESSAGE_TYPES = {
    "QUERY_STANDINGS_REQUEST",     # Player → Manager
    "QUERY_SCHEDULE_REQUEST",      # Player → Manager
    "LEAGUE_UNREGISTER_REQUEST",   # Player → Manager
    "MATCH_RESULT_REPORT",         # Referee → Manager
    # REMOVED: "GAME_JOIN_ACK", "CHOOSE_PARITY_RESPONSE" (player → referee)
}
```

**File**: `SHARED/league_sdk/auth.py`

### Bug Fix #4: Empty Standings Table

**Problem**: Round completed but standings table displayed headers with no data rows.

**Root Cause**: Display code used `s['won']`, `s['draw']`, `s['lost']` but standings data has keys `s['wins']`, `s['draws']`, `s['losses']` (plural).

**Solution**: Use correct plural keys:
```python
# BEFORE (broken)
print(f"{s['won']:<5} {s['draw']:<5} {s['lost']:<5}")  # KeyError!

# AFTER (fixed)
print(f"{s['wins']:<5} {s['draws']:<6} {s['losses']:<7}")
```

**File**: `agents/league_manager/handlers.py`

### Bug Fix #5: GAME_OVER Never Sent to Players

**Problem**: Matches completed but players never received results, so adaptive strategies couldn't learn.

**Root Cause**: `_send_game_over()` method only logged a message but didn't actually send HTTP requests to players.

**Solution**: Implement actual HTTP sends:
```python
async def _send_game_over(self, match_state: dict, result: dict) -> None:
    client = MCPClient(f"referee:{self.referee.referee_id}", self.referee.auth_token)

    game_over_a = {"match_id": ..., "drawn_number": ..., "result": ...}
    game_over_b = {"match_id": ..., "drawn_number": ..., "result": ...}

    await asyncio.gather(
        client.send(match_state["player_a"]["endpoint"], "GAME_OVER", game_over_a),
        client.send(match_state["player_b"]["endpoint"], "GAME_OVER", game_over_b),
    )
```

**File**: `agents/referee_template/game_logic.py`

### Bug Fix #6: Duplicate Log Directories

**Problem**: Logs appeared in both `SHARED/logs/player/` and `SHARED/logs/players/`, same for referees.

**Root Cause**: Agent's `main.py` used `JsonLogger("players", id)` but `MCPServer` used `JsonLogger("player", id)`.

**Solution**: Standardize agent_type in both places:
```python
# In agent main.py
self.server = MCPServer("players", player_id, ...)  # Plural
self.logger = JsonLogger("players", player_id)      # Plural
```

**Files**: `agents/player_template/main.py`, `agents/referee_template/main.py`, `agents/league_manager/main.py`

### Key Takeaways

1. **Always capture HTTP responses** - Don't just fire-and-forget async calls
2. **Auth requires registration** - Generating tokens isn't enough, must call `register_auth_token()`
3. **Auth should be asymmetric** - Not all agents have all tokens
4. **Data structure consistency** - Match keys between data producers and consumers
5. **Implement what you log** - If logs say "sending X", actually send X
6. **Standardize naming** - Inconsistent names cause subtle bugs
7. **Test both modes** - Simulation ≠ Distributed, always test HTTP flow
