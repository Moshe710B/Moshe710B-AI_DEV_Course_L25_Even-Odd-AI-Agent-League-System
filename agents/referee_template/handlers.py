        ...
        async def run_match():
            ...
            try:
                ...
                self.logger.info(
                    ...
                    match_id=match_id,
                    result=result.get("winner_id", "DRAW"),
                )
                ...
                self.logger.error(
                    "MATCH_ERROR",
                    f"Error in match {match_id}: {e}",
                    match_id=match_id,
                )

        # Launch match in background (don't await)
        asyncio.create_task(run_match())

        # Return immediately to League Manager
        ...
        try:
            await client.send(
                ...
                "MATCH_RESULT_REPORT",
                {
                    ...
                    "player_a_id": result.get("player_a_id"),
                    "player_b_id": result.get("player_b_id"),
