...
class InvitationHandler:
    ...
    async def send_invitations(self, match_state: dict) -> bool:
        ...
        self.logger.info(
            "SENDING_INVITATIONS",
            f"Sending invitations for match {match_id}",
            match_id=match_id,
        )
        ...
        async def invite_player(player: dict, role: str, opponent_id: str) -> bool:
            try:
                self.logger.info(
                    "INVITING_PLAYER",
                    f"Inviting {player['id']} to {player['endpoint']}",
                    player_id=player["id"],
                ...
                self.logger.info(
                    "INVITATION_SENT",
                    f"Invitation sent to {player['id']}",
                    player_id=player["id"],
                )
                return True
        ...
        self.logger.info(
            "WAITING_FOR_JOINS",
            f"Waiting for players to join match {match_id}",
            match_id=match_id,
        )
        await asyncio.sleep(5)

        joined = match_state["player_a_joined"] and match_state["player_b_joined"]
        self.logger.info(
            "JOIN_CHECK",
            f"Match {match_id}: A={match_state['player_a_joined']}, B={match_state['player_b_joined']}",
            match_id=match_id,
            both_joined=joined,
        )
