...
class LeagueManagerHandlers:
    ...
    async def complete_league(self) -> None:
        ...
        # Print final standings to console
        print("\n" + "="*60)
        print("           FINAL STANDINGS")
        print("="*60)
        print(f"{'Rank':<6} {'Player':<10} {'Played':<7} {'W':<4} {'D':<4} {'L':<4} {'Points':<7}")
        print("-"*60)
        for s in standings:
            print(f"{s['rank']:<6} {s['player_id']:<10} {s['played']:<7} {s['wins']:<4} {s['draws']:<4} {s['losses']:<4} {s['points']:<7}")
        print("="*60)
        if winner:
            print(f"🏆 CHAMPION: {winner['player_id']} ({winner['display_name']}) - {winner['points']} points")
        print("="*60 + "\n")
