...
# Only messages FROM players/referees TO manager need auth validation
# Messages FROM manager/referees TO players should NOT be validated
# Messages FROM players TO referees should NOT be validated (referees don't have player tokens)
AUTH_REQUIRED_MESSAGE_TYPES: Set[str] = {
    # Player -> Manager messages (manager knows player tokens)
    "QUERY_STANDINGS_REQUEST",  # Player -> Manager (player proves identity)
    "QUERY_SCHEDULE_REQUEST",  # Player -> Manager (player proves identity)
    "LEAGUE_UNREGISTER_REQUEST",  # Player -> Manager (player proves identity)

    # Referee -> Manager messages (manager knows referee tokens)
    "MATCH_RESULT_REPORT",  # Referee -> Manager (referee proves identity)

    # NOTE: REMOVED player->referee messages because referees don't have player tokens:
    # - GAME_JOIN_ACK: Player -> Referee (removed)
    # - CHOOSE_PARITY_RESPONSE: Player -> Referee (removed)

    # NOTE: These are FROM referee TO player, so players can't validate referee's token:
    # - GAME_INVITATION
    # - CHOOSE_PARITY_CALL
    # - GAME_OVER
}
